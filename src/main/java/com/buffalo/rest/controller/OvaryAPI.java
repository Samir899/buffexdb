...
@RestController
@RequestMapping("/ovary/api") 
public class OvaryAPI { 
  
	@Autowired
    private OvaryService ovaryService;
  
    public OvaryAPI(OvaryService productService) 
    { 
        this.ovaryService = productService; 
    }
    
    @GetMapping("/ovarys")  
    public List<Ovary> getAllOvary() 
    { 
    	return ovaryService.listAllOvary();
    }
    
    @GetMapping("/ovarys/sum")  
